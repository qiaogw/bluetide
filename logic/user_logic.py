...
#
#
_query_sql = '''select u.user_key ,u.user_id,u.user_name, u.cust_key, u.dept_key, u.region_key, u.user_tel, u.user_tel2,
    u.user_email, u.user_status, u.user_seq, u.user_lastLoginTime, u.user_lastLoginIp, u.user_loginCount,
    u.user_beginDate, u.user_endDate, u.remark, u.isDelete, u.creater, u.createTime, u.lastUpdater, u.lastUpdateTime,
    d.dept_name, c.cust_name, r.region_name
    from (( wqt_user as u inner JOIN wqt_dept as d ON d.dept_key = u.dept_key) inner join wqt_cust as c on
    u.cust_key = c.cust_key) inner join wqt_region as r on u.region_key = r.region_key
    where u.isDelete = %s  '''
_query_col = str_helper.format_str_to_list_filter_empty('user_key,user_id,user_name,cust_key,dept_key,region_key,user_tel,\
    user_tel2,user_email,user_status,user_seq, user_lastLoginTime, user_lastLoginIp, user_loginCount,user_beginDate, \
    user_endDate, remark, isDelete, creater, createTime, lastUpdater, lastUpdateTime, dept_name,cust_name, region_name',
                                                        ',')
''' 分页查询用户信息 '''
...
#
# ''' 根据userID查询用户 '''
def query_one_by_id(user_id = ''):
    sql = _query_sql
    isdelete = state.Boole['false']
    ps = [isdelete]
    if 0 != user_id:
        sql = sql + ' and u.user_id = %s '
        ps.append(user_id)
    else:
        return None
    yz = tuple(ps)
...
#     return user
#
_add_sql = '''   INSERT INTO wqt_user (cust_key, region_key, dept_key, user_name, user_password, user_id,
 user_tel, user_tel2, user_email, user_status, user_seq, user_lastLoginTime, user_lastLoginIp, user_loginCount,
  user_beginDate, user_endDate, remark, isDelete, creater, createTime, lastUpdater, lastUpdateTime)
VALUES(%s, %s, %s, %s, %s, %s, %s, %s ,%s, %s ,%s, %s, null, null, 0, %s, %s, %s, %s, %s, now(), %s, now() )  '''
''' 创建用户 '''
